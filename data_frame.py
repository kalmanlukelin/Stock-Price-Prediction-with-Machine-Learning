...
def get_self_made_data_frame(symbol, start_date, end_date):
    date_range = pd.date_range(start_date, end_date)
    data_frame = pd.DataFrame(index = date_range)
    ...
    scaler = MinMaxScaler()
    data_frame[['stock value', 'b_value', 'momentum', 'volatility']] =  scaler.fit_transform(data_frame[['stock value', 'b_value', 'momentum', 'volatility']])
